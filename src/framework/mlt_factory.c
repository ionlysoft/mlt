...
#  define PREFIX_LIB "\\mlt"
   /** the default subdirectory of the install prefix for holding module (plugin) data */
#  define PREFIX_DATA "\\plugins\\mlt"
#  define PROFILES_DATA "\\opts\\profiles"
#else
...
MLTPP_DECLSPEC mlt_repository mlt_factory_init( const char *directory )
{
	...
	if ( global_properties )
	{
        ...
        char propath[1024];
        strcpy( propath, appdir );
        strcat( propath, PROFILES_DATA);
        mlt_properties_set( global_properties, "MLT_PROFILES_PATH",propath);
...
MLTPP_DECLSPEC mlt_producer mlt_factory_producer( mlt_profile profile, const char *service, const void *resource )
{
...
MLTPP_DECLSPEC mlt_filter mlt_factory_filter( mlt_profile profile, const char *service, const void *input )
{
	mlt_filter obj = NULL;
...
MLTPP_DECLSPEC mlt_consumer mlt_factory_consumer( mlt_profile profile, const char *service, const void *input )
{
...
MLTPP_DECLSPEC void mlt_factory_close( )
{
	if ( mlt_directory != NULL )
	{
