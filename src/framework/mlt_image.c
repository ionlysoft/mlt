...
MLTPP_DECLSPEC const char * mlt_image_format_name( mlt_image_format format )
{
	switch ( format )
	{
...
/** Get the number of bytes needed for an image.
  ...
  * \return the number of bytes
  */
MLTPP_DECLSPEC int mlt_image_format_size( mlt_image_format format, int width, int height, int *bpp )
{
	switch ( format )
	{
		case mlt_image_rgb:
 ...
 */
MLTPP_DECLSPEC void mlt_image_format_planes( mlt_image_format format, int width, int height, void* data, uint8_t* planes[4], int strides[4])
{
