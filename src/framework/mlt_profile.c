...
{
	char *filename = NULL;
    const char *prefix = mlt_environment("MLT_PROFILES_PATH");//getenv( "MLT_PROFILES_PATH" );
...
MLTPP_DECLSPEC mlt_profile mlt_profile_init( const char *name )
{
	mlt_profile profile = NULL;
...
MLTPP_DECLSPEC void mlt_profile_close( mlt_profile profile )
{
	if ( profile )
	{
...
MLTPP_DECLSPEC mlt_profile mlt_profile_clone( mlt_profile profile )
{
...
MLTPP_DECLSPEC mlt_properties mlt_profile_list( )
{
	char *filename = NULL;
...
MLTPP_DECLSPEC void mlt_profile_from_producer( mlt_profile profile, mlt_producer producer )
{
	mlt_frame fr = NULL;
	uint8_t *buffer = NULL;
