...
MLTPP_DECLSPEC mlt_service mlt_service_consumer( mlt_service self )
{
	// Get the service base
	mlt_service_base *base = self->local;
...
MLTPP_DECLSPEC int mlt_service_attach( mlt_service self, mlt_filter filter )
{
...
MLTPP_DECLSPEC void mlt_service_cache_set_size( mlt_service self, const char *name, int size )
{
	mlt_cache cache = get_cache( self, name );
	if ( cache )
