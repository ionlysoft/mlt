/*
 * filter_watermark_affine.c -- watermark_affine filter
...
static int filter_get_image( mlt_frame frame, uint8_t **image, mlt_image_format *format, int *width, int *height, int writable )
{
	...
	// Get the watermark_affine filter object
    ...
    mlt_transition affine = mlt_properties_get_data( properties, "affine", NULL );
    ...
    if ( affine == NULL )
	{
        ...
        affine = mlt_factory_transition( profile, "affine", NULL );

		// Register the composite for reuse/destruction
        if ( affine != NULL )
            mlt_properties_set_data( properties, "affine", affine, 0, ( mlt_destructor )mlt_transition_close, NULL );
	}
	...
	{
		// Get the properties
        mlt_properties composite_properties = MLT_TRANSITION_PROPERTIES( affine );

		// Pass all the composite. properties on the filter down
        mlt_properties_pass( composite_properties, properties, "affine." );

        if ( mlt_properties_get( properties, "affine.out" ) == NULL )
    ...
    *format = mlt_image_rgba;
    ...
    if ( !error && affine != NULL && producer != NULL )
	{
		...
		if ( mlt_service_get_frame( service, &b_frame, 0 ) == 0 )
		{
			...
			{
                mlt_properties_set_int( MLT_TRANSITION_PROPERTIES( affine ), "distort", 1 );
            ...
            *format = mlt_image_rgba;
			...
			{
                mlt_properties_set( a_props, "rescale.interp", "bilinear" );

				// Apply all filters that are attached to this filter to the b frame
				mlt_service_apply_filters( MLT_FILTER_SERVICE( filter ), b_frame, 0 );

				// Process the frame
                mlt_transition_process( affine, a_frame, b_frame );
			...
			else
			{
				...
				if ( rescale == NULL || !strcmp( rescale, "none" ) )
                    rescale = "bilinear";
                mlt_transition_process( affine, b_frame, a_frame );
...
mlt_filter filter_watermark_affine_init( mlt_profile profile, mlt_service_type type, const char *id, char *arg )
{
